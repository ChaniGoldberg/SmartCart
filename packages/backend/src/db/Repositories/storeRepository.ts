...
import { SupabaseClient } from "@supabase/supabase-js";
import { IStoreRepository } from "../IRepositories/IStoreRepository";
import { Store } from "@smartcart/shared";export class StoreRepository implements IStoreRepository {
    private readonly tableName = 'store';    constructor(private supabase: SupabaseClient) { }    // פונקציה להמרה ל-snake_case
    private toDbStore(store: Store) {
        return {
            store_pk: store.storePK, // Assuming storePK is a unique identifier for the store
            store_id: store.storeId,
  ...
  }  async getAllStoresMinimal(): Promise<{
    store_pk: string;
    latitude?: number;
    longitude?: number;
    address: string;
    ...
    const { data, error } = await this.supabase
      .from(this.tableName)
      .select('store_pk, latitude, longitude, address, city');    if (error) {
