...
function convertRawPromotionToPromotion(
  ...
  // שינוי: promotionItemsCode יהיה מערך של string
  let promotionItemsCode: string[] = [];
  if (rawPromo?.PromotionItems?.[0]?.Item) {
    const items = rawPromo.PromotionItems[0].Item;
    if (Array.isArray(items)) {
      promotionItemsCode = items.map((item: any) => String(item.ItemCode?.[0] ?? ""));
    } else if (items?.ItemCode?.[0]) {
      promotionItemsCode = [String(items.ItemCode[0])];
    }
  }
  return {
    promotionId: parseInt(rawPromo?.PromotionId?.[0] ?? "0"),
    storePK: storeId ?? "", // במקום storeId: parseInt(...),
    ...
    promotionItemsCode, // עכשיו זה string[]
    minQuantity: parseFloat(rawPromo?.MinQty?.[0] ?? "0"),
    maxQuantity: rawPromo?.MAXQTY?.[0] ? parseFloat(rawPromo.MAXQTY[0]) : 0,
